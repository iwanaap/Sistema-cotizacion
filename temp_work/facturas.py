...
def calcular_fecha_vencimiento(fecha_str, condiciones_pago):
    if condiciones_pago == "30 días":
...
def lista_facturas():
    ...
    # Convertir los resultados y agregar fecha de vencimiento
    facturas = []
    today = datetime.now().strftime("%Y-%m-%d")
    for factura in facturas_raw:
        fecha_vencimiento = calcular_fecha_vencimiento(factura[6], factura[4]) if factura[6] and factura[4] else None
        factura_list = list(factura)
        factura_list.append(fecha_vencimiento)
        facturas.append(factura_list)
    ...
    return render_template('facturas.html', facturas=facturas, search_query=search_query, today=today)
